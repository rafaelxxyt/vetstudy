  ...
  return (
    <>
      <motion.aside
        ...
        className="hidden md:flex md:flex-col bg-slate-900 border-r border-slate-800 overflow-hidden flex-shrink-0 z-20 relative"
      >
        {/* ── Cabeçalho / Toggle ─────────────────────────── */}
      ...
      <nav className="md:hidden fixed inset-x-0 bottom-0 z-30 border-t border-slate-800 bg-slate-900/95 backdrop-blur">
        <div className="grid grid-cols-5 gap-1 px-2 py-2">
          <MobileNavBtn item={{ id: 'hub', label: 'Hub', icon: GraduationCap }} active={activePage === 'hub'} onClick={() => onNavigate('hub')} />
          {CLINICA.map(item => (
            <MobileNavBtn key={item.id} item={item} active={activePage === item.id} onClick={() => onNavigate(item.id)} />
          ))}
        </div>
      </nav>
    </>
...
function MobileNavBtn({ item, active, onClick }: {
  ...
  return (
    <button
      onClick={onClick}
      className={`flex min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[10px] font-semibold transition-all ${
        active
          ? 'bg-teal-500/15 text-teal-300 border border-teal-500/25'
          : 'text-slate-500 border border-transparent hover:bg-slate-800 hover:text-slate-200'
      }`}
    >
      <Icon size={16} className="flex-shrink-0" />
