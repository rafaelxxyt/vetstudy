...
function RevisaoContent({ profileId }: { profileId?: string }) {
    ...
    <div className="p-8 max-w-2xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-2xl font-bold text-white">RevisÃ£o Master</h1>
        </div>
        <p className="text-slate-400 text-sm">Flashcards do Estudo Dirigido + banco clÃ­nico Â· memorizaÃ§Ã£o pura</p>
      </div>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-600">{flipped ? 'Resposta visÃ­vel' : 'Toque para ver'}</span>
      ...
      <div className="flex items-center justify-between mt-6">
        <button onClick={() => goTo(Math.max(0, index - 1))} disabled={index === 0}
          className="flex items-center gap-2 px-5 py-2.5 bg-slate-800 border border-slate-700 text-slate-300 rounded-xl text-sm font-semibold hover:border-teal-500/40 hover:text-white disabled:opacity-30 transition-all">
        ...
        <button onClick={() => goTo(Math.min(deck.length - 1, index + 1))} disabled={index === deck.length - 1}
          className="flex items-center gap-2 px-5 py-2.5 bg-teal-600 text-white rounded-xl text-sm font-semibold hover:bg-teal-700 disabled:opacity-30 transition-all">
...
export default function RevisaoPage({ profileId }: { profileId?: string } = {}) {
  const resolvedProfileId = profileId ?? getActiveProfile()?.id
  return (
    <Gatekeeper pageTitle="RevisÃ£o Master">
      <RevisaoContent profileId={resolvedProfileId} />
    </Gatekeeper>
  )
}
